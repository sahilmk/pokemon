...
import { Inputcomp, Buttoncomp } from '../../stories'
...
function Registerpage() {
    ...
    const validate = (e?: FormType) => {
        ...
        if (!e?.useremail) {
            ...
            if (emailRegex.test(e.useremail) === false || e.useremail.length <= 4 || e.useremail.length > 100) {
        ...
        if (!e?.password) {
            ...
            if (passwordRegex.test(e.password) === false || e.password.length <= 4 || e.password.length >= 16) {
                errors.password = 'Password must contain atleast one number and length must be between 5 to 15 character'
            }
        }
                                    ...
                                    {({ input, meta }) => (
                                        <div>
                                            ...
                                            {meta.error && meta.touched && <span
                                                className={style.errorm}>{meta.error}</span>}
                                        </div>
                                    )}
                                </Field>
                            </div>
